...
final class IPYTW_Plugin_Upd {
	const API_URL = 'https://icopydoc.ru/api/v1';
	private $list_plugin_names = [ 
		'ipytwp' => [ 'name' => 'PRO', 'code' => 'renewlicense22imp' ]
	];
	...
	public function __construct( $args = [] ) {
		...
		$this->list_plugin_names = apply_filters( 'ipytw_f_list_plugin_names', $this->list_plugin_names, $args );
		do_action_ref_array( 'ipytw_a_plugin_upd', $args );
	...
	public function add_plugin_action_links( $actions, $plugin_file ) {
		...
		switch ( $i ) {
			case "202":
				$message = __( 'License is active', 'import-from-yml' );
				$color = 'green';
				break;
			case "402":
				$message = __( 'License expired', 'import-from-yml' );
				$color = '#dc3232';
				break;
			case "412":
				$message = __( 'License data is invalid', 'import-from-yml' );
				$color = '#dc3232';
				break;
			case "418":
				$message = __(
	...
	public function get_info() {
		...
		switch ( $c ) {
			...
			case "402":
				$message = sprintf(
					'<span style="font-weight: 700;">Import from YML %1$s:</span> %2$s! %3$s, <a href="https://icopydoc.ru/product/%4$s/?utm_source=%4$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=notice&utm_term=license-expired" target="_blank">%5$s</a> (%6$s: <span style="font-weight: 700;">%7$s</span>). %8$s <a href="/wp-admin/admin.php?page=ipytwexport">%9$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'License expired', 'import-from-yml' ),
					__( 'Please', 'import-from-yml' ),
					$this->get_slug(),
					__( 'purchase a new license', 'import-from-yml' ),
					__( 'to get a discount, use this promo code', 'import-from-yml' ),
					$this->list_plugin_names[ $this->get_pref()]['code'],
					__( 'If you have already done this', 'import-from-yml' ),
					__( 'enter the new license information here', 'import-from-yml' )
				);
				break;
			case "412":
				$message = sprintf( '<span style="font-weight: 700;">Import from YML %1$s:</span> %2$s! %1$s %3$s. <a href="/wp-admin/admin.php?page=ipytwexport">%4$s</a> %5$s <a href="https://icopydoc.ru/product/%6$s/?utm_source=%6$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=license-err&utm_term=notice" target="_blank">%7$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'License data is invalid', 'import-from-yml' ),
					__( 'version features do not work and you can not install updates', 'import-from-yml' ),
					__( 'Enter your license information', 'import-from-yml' ),
					__( 'or', 'import-from-yml' ),
					$this->get_slug(),
					__( 'purchase a new one', 'import-from-yml' )
				);
				break;
			case "418":
				$message = sprintf( '<span style="font-weight: 700;">Import from YML %1$s:</span> %2$s! <a href="/wp-admin/admin.php?page=ipytwexport">%3$s</a> %4$s <a href="https://icopydoc.ru/product/%5$s/?utm_source=%5$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=license-limit&utm_term=notice" target="_blank">%6$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__(
						'This license cannot be used on this site. The package limit has been exceeded',
						'import-from-yml'
		...
		if ( $c !== '0' ) {
			...
			if ( $remaining_days > 0 && $remaining_days < 8 ) {
				$message = sprintf(
					'<span style="font-weight: 700;">Import from YML %1$s:</span> %2$s <span style="font-weight: 700; color: red;">%3$s</span>. %4$s, <a href="https://icopydoc.ru/product/%5$s/?utm_source=link&utm_medium=organic&utm_campaign=in-plugin&utm_content=notice&utm_term=license-remaining" target="_blank">%6$s</a> (%7$s: <span style="font-weight: 700;">%8$s</span>). %9$s <a href="/wp-admin/admin.php?page=ipytwexport">%10$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'License expires in', 'import-from-yml' ),
					$this->num_decline( $remaining_days, [ 
						__( 'day', 'import-from-yml' ),
						_x( 'days', '2 days', 'import-from-yml' ),
						_x( 'days', '5 days', 'import-from-yml' )
					]
					),
					__( 'Please', 'import-from-yml' ),
					$this->get_slug(),
	 ...
	 *
	 * Примеры вызова:
	 * ipytwp_num_decline($num, 'книга,книги,книг')
	 * ipytwp_num_decline($num, ['книга','книги','книг'])
	 * ipytwp_num_decline($num, 'книга', 'книги', 'книг')
	 * ipytwp_num_decline($num, 'книга', 'книг')
	 *
	...
	private function get_body_request() {
		$body_request = [ 
			...
			'basic_version' => IPYTW_PLUGIN_VERSION,
			'license_key' => $this->get_license_key(),
			'order_id' => $this->get_order_id(),
			'order_email' => $this->get_order_email(),
			'order_home_url' => home_url( '/' ),
		];
		new IPYTW_Error_Log( $body_request );
		return $body_request;
	}
	...
	private function response_to_api() {
		...
		$api_url = apply_filters( 'ipytw_f_api_url', self::API_URL );
		...
		if ( isset( $options['hook_extra']['plugin'] ) ) {
			if ( $options['hook_extra']['plugin'] === $this->plugin_slug ) {
				...
				$package_url = sprintf(
					...
					IPYTW_PLUGIN_VERSION
				);
				$package_url = apply_filters( 'ipytw_f_package_url', $package_url, $options );
	...
	/**
	 * Summary of get_plugin_response_data
	 * 
	 * @param mixed $resp
	 * 
	 * @return stdClass
