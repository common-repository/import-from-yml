...
// 'import-from-yml' - slug for translation (be sure to make an autocorrect)
if ( ! class_exists( 'ICPD_Feedback' ) ) {
	final class ICPD_Feedback {
		...
		public function init_hooks() {
			...
			if ( isset( $_REQUEST[ $this->get_submit_name()] ) ) {
				...
				add_action( 'admin_notices', function () {
					...
					$message = __( 'The data has been sent. Thank you', 'import-from-yml' );
		...
		public function print_view_html_feedback_block() { ?>
			<div class="postbox">
				<h2 class="hndle">
					<?php esc_html_e( 'Send data about the work of the plugin', 'import-from-yml' ); ?>
				</h2>
				<div class="inside">
					<?php
					printf( '<p>%s! %s:</p>',
						esc_html__( 'Sending statistics you help make the plugin even better', 'import-from-yml' ),
						esc_html__( 'The following data will be sent', 'import-from-yml' )
					);
					?>
					<ul class="icpd_ul">
						<?php
						printf( '<li>%s</li><li>%s</li><li>%s %s</li></p>',
							esc_html__( 'PHP version information', 'import-from-yml' ),
							esc_html__( 'Multisite mode status', 'import-from-yml' ),
							esc_html__( 'Technical information and plugin logs', 'import-from-yml' ),
					...
					<form action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" method="post" enctype="multipart/form-data">
						<?php
						printf( '<p>%s (%s)?</p>',
							esc_html__( 'Did the plugin help you', 'import-from-yml' ),
							esc_html( $this->get_plugin_name() )
						);
						?>
						<p>
							...
							printf( '<input type="radio" value="yes" name="%s">%s<br />',
								...
								esc_html__( 'Yes', 'import-from-yml' )
							);

							printf( '<input type="radio" value="no" name="%s">%s<br />',
								esc_attr( $this->get_radio_name() ),
								esc_html__( 'No', 'import-from-yml' )
							);
							?>
						</p>
						<p>
						...
						<p>
							<?php esc_html_e( 'Your message', 'import-from-yml' ); ?>:
						</p>
						<p><textarea rows="6" cols="32" name="<?php echo $this->get_textarea_name(); ?>" placeholder="<?php
						   printf( '%1$s (%2$s). %3$s',
						   	esc_attr( esc_html__( 'Enter your text to send me a message', 'import-from-yml' ) ),
						   	esc_attr( esc_html__( 'You can write me in Russian or English', 'import-from-yml' ) ),
						   	esc_attr( esc_html__( 'I check my email several times a day', 'import-from-yml' ) )
						   ); ?>"></textarea></p>
						...
						<input class="button-primary" type="submit" name="<?php echo $this->get_submit_name(); ?>"
							value="<?php esc_html_e( 'Send data', 'import-from-yml' ); ?>" />
					</form>
				</div>
			</div>
			<?php
